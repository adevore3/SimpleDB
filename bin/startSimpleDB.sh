...
do 
	...
	then
		if ! xEnabled 
		then
		    (exec ssh $host "java -version; cd /tmp/simpledb; java -classpath \
		...
		osascript -e "tell app \"Terminal\"
			do script \"echo -e \\\"\\\\033]0;Worker: $host:$port. Do not close this window when SimpleDB is running.\\\\007\\\"; ssh $host \\\"cd /tmp/simpledb; java -version; java -classpath \\\\\\\"bin/src:lib/*\\\\\\\" simpledb.parallel.Worker ${host}:$port $serverAddr \\\" \"
		end tell"
	fi
