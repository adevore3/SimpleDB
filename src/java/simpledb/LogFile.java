 ...
 * their first log record on disk. The format of the record is an integer count of the number of
 * transactions, as well as a long integer transaction id and a long integer first record offset for
...
public class LogFile
{
     ...
     * whether the caller is creating a brand new DB, in which case we should ignore the log file,
     * or whether the caller will eventually want to recover (after populating the Catalog). So we
     * make this decision lazily: if someone calls recover(), then do it, while if someone starts
     * adding log file entries, then first throw out the initial log file contents.
     * 
     * @param f
     *            The log file's name
     */
            ...
            throws IOException
    {
         ...
         * 
         * record type transaction id before page data (see writePageData) after page data start
         * offset
         */
    ...
    void writePageData(RandomAccessFile raf, Page p) throws IOException
    {
        ...
        raf.write(pageData);
        // Debug.log ("WROTE PAGE DATA, CLASS = " + pageClassName + ", table = " + pid.getTableId()
        // + ", page = " + pid.pageno());
    }
    ...
    public synchronized void logTruncate() throws IOException
    {
        ...
        Debug.log("TRUNCATING LOG;  WAS " + raf.length()
                + " BYTES ; NEW START : " + minLogRecord + " NEW LENGTH: "
     ...
     */
    public void rollback(TransactionId tid) throws NoSuchElementException,
            IOException
    {
    ...
    public void rollback(long tid) throws NoSuchElementException, IOException
    {
        synchronized (Database.getBufferPool())
        {
            synchronized (this)
            {
                ...
                raf.seek(this.tidToFirstLogRecord.get(tid));
                ...
                while (raf.getFilePointer() <= raf.length())
                {
                    try
                    {
                        ...
                        case UPDATE_RECORD:
                            ...
                            Page beforeImage = this.readPageData(raf);
    ...
    public void recover() throws IOException
    {
        synchronized (Database.getBufferPool())
        {
            synchronized (this)
            {
                ...
                while (raf.getFilePointer() <= raf.length())
                {
                    try
                    {
                        ...
                        case ABORT_RECORD:
                            ...
                            raf.seek(current_offset + LONG_SIZE);
                            break;

                        case COMMIT_RECORD:
                            this.tidToFirstLogRecord.remove(raf.readLong());
                            raf.seek(raf.getFilePointer() + LONG_SIZE);
                            break;
                        ...
                        case UPDATE_RECORD:
                            raf.readLong();
                            this.readPageData(raf);
                            Page afterImage = this.readPageData(raf);
                            Database.getCatalog()
                                    .getDbFile(afterImage.getId().getTableId())
                                    .writePage(afterImage);
                            raf.readLong();
                            Database.getBufferPool().discardPage(afterImage.getId());
                            break;

                        case BEGIN_RECORD:
                            this.tidToFirstLogRecord.put(raf.readLong(),
                                    raf.readLong());
                            break;
                        ...
                        case CHECKPOINT_RECORD:
                            ...
                            for (int i = 0; i < numActiveTrans; i++)
                                this.tidToFirstLogRecord.put(raf.readLong(), raf.readLong());
                ...
                for (Long key : this.tidToFirstLogRecord.keySet())
                {
                    this.rollback(key);
                }
    ...
    public void print() throws IOException
    {
        ...
        System.out.println("Last written checkpoint: " + raf.readLong());

        while (raf.getFilePointer() < raf.length())
        {
            System.out.print("File offset: " + raf.getFilePointer()
                    + "; Record type: ");
            ...
            {
            case ABORT_RECORD:
                System.out.println("ABORT RECORD");
                System.out.println("TransactionId: " + raf.readLong());
                break;
            ...
            case UPDATE_RECORD:
                ...
                System.out.println("\tPage: "
                        + this.readPageData(raf).getId().pageNumber());
            ...
            case CHECKPOINT_RECORD:
                ...
                {
                    System.out.println("\tTransactionId: " + raf.readLong());
                    System.out.println("\tFirst record offset "
                            + raf.readLong());
                }
                break;
                
            default:
                System.out.println("SHOULDN'T BE HERE EVER!!!!!!!!");
                break;
            }

            System.out.println("Record began at : " + raf.readLong() + "\n");
        }
    }
